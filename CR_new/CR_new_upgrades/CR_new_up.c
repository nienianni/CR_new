...
void load_write_file()
{
	...
	double S;
	errno_t err1;
	err1 = fopen_s(&fp3, "c_alpha_I_L_pd.txt", "w");
	//errno_t err2;
	...
	//	puts("不能打开文件sus");
	//}
	for (alpha = 0.5; alpha <= 0.5; alpha = alpha + 0.2)
	{
...
/*void load_write_file()
{
	...
	double S;
	errno_t err1;
	err1 = fopen_s(&fp3, "c_pd_I_L_alpha.txt", "w");
	//errno_t err2;
	//err2 = fopen_s(&fp2, "help2.txt", "w");
	if (err1 != 0)
	{
